...
def generate_dataset(args, partition):
    ...
    if isinstance(dataset, ConcatDataset):
        ...
        # Handle SVHN's different attribute name for labels
        labels = []
        for i in range(len(dataset.datasets)):
            if hasattr(dataset.datasets[i], 'targets'):
                labels.append(dataset.datasets[i].targets)
            elif hasattr(dataset.datasets[i], 'labels'):  # For SVHN
                labels.append(dataset.datasets[i].labels)
...
def load_dataset(args, download=DOWNLOAD):
    ...
    if args.dataset == "cifar10":
        ...
        train_dataset = torchvision.datasets.SVHN(
            root=FULL_PATH, split="train", download=download, transform=None)
        test_dataset = torchvision.datasets.SVHN(
            root=FULL_PATH, split="test", download=download, transform=None)
