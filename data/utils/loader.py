...
from torchvision.datasets import MNIST, FashionMNIST, CIFAR10, CIFAR100, ImageFolder, SVHN
...
def get_partition(args, indices=None):
    ...
    if args.dataset in [ "emnist", "fmnist"]:
        ...
        test_transform = t.Compose([
            ...
            t.ToTensor(),
            t.Lambda(lambda x: x.repeat(3, 1, 1)),  # 复制为三通道
            t.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5))
        ])
    else:
        train_transform = t.Compose([
    ...
    if args.dataset in ["cifar10", "cifar100", "mnist", "fmnist", "tinyimagenet", "svhn"]:
...
def load_svhn(data_path, download=DOWNLOAD):
    train_set = SVHN(data_path, train=True, transform=None, download=download)
    test_set = SVHN(data_path, train=False, transform=None, download=download)
    full_set = ConcatDataset([train_set, test_set])
    return full_set
